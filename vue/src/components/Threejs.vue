  ...
  methods: {
    init: function () {
      ...
      function canWalk(x, y) {
        if (x >= 0 && x < tailleTab && y >= 0 && y < tailleTab && tab[x][y] !== 'lave') {
          return true
        } else {
          return false
        }
      ...
      let tabToExplore = [[x, y]]
      let tabHasBeenExplored = []

      while (tabToExplore.length) {
        const activeCase = tabToExplore[0]
        if (tab[activeCase[0]][[activeCase[1]]] === 'key') {
          console.log('Key Found')
          break
        }

        if (canWalk(activeCase[0] + 1, activeCase[1]) && !tabHasBeenExplored.some(cell => cell[0] === activeCase[0] + 1 && cell[1] === activeCase[1]) && !tabToExplore.some(cell => cell[0] === activeCase[0] + 1 && cell[1] === activeCase[1])) {
          tabToExplore.push([activeCase[0] + 1, activeCase[1]])
        }
        if (canWalk(activeCase[0] - 1, activeCase[1]) && !tabHasBeenExplored.some(cell => cell[0] === activeCase[0] - 1 && cell[1] === activeCase[1]) && !tabToExplore.some(cell => cell[0] === activeCase[0] - 1 && cell[1] === activeCase[1])) {
          tabToExplore.push([activeCase[0] - 1, activeCase[1]])
        }
        if (canWalk(activeCase[0], activeCase[1] + 1) && !tabHasBeenExplored.some(cell => cell[0] === activeCase[0] && cell[1] === activeCase[1] + 1) && !tabToExplore.some(cell => cell[0] === activeCase[0] && cell[1] === activeCase[1] + 1)) {
          tabToExplore.push([activeCase[0], activeCase[1] + 1])
        }
        if (canWalk(activeCase[0], activeCase[1] - 1) && !tabHasBeenExplored.some(cell => cell[0] === activeCase[0] && cell[1] === activeCase[1] - 1) && !tabToExplore.some(cell => cell[0] === activeCase[0] && cell[1] === activeCase[1] - 1)) {
          tabToExplore.push([activeCase[0], activeCase[1] - 1])
        }
        ...
        tabToExplore.shift()
        tabHasBeenExplored.push(activeCase)
      }

      HighlightCheckCase(this.scene, tabHasBeenExplored, 0x333333)
